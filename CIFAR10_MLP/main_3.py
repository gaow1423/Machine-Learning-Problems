...
#    print (k)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        if (batch_idx <= 999):
            ...
            optimizer = optim.SGD(model.parameters(), lr = 0.1, momentum = k)
            optimizer.zero_grad()
            output = model(data)
...
#                return F.log_softmax(self.fc2(x), 1)
        ...
        l = range(1, epochs + 1)
        plt.subplot(1, 3, 1)
        plt.plot(l, accv, label = "Training Accuracy with momentum = %f"%(k))
        plt.title('Training Accuracy')
        plt.xlabel('x-axis: the Number of Epochs')
        plt.ylabel('y-axis: Accuracy of Validation set (%)')
        
        plt.subplot(1, 3, 2)
        plt.plot(l, lossv, label = "Training Loss with momentum = %f"%(k))
        plt.title('Training loss curve')
        plt.xlabel('x-axis: the Number of Epochs')
        plt.ylabel('y-axis: Training loss')
       
        plt.subplot(1, 3, 3)
        plt.plot(l, accv_t, label = "Test Accuracy with momentum = %f"%(k))
